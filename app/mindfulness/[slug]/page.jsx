...
import { getMindfulnessPage, getAllMindfulnessSlugs } from '../../../lib/mindfulness';
import { notFound } from 'next/navigation';

export async function generateStaticParams() {
  const slugs = getAllMindfulnessSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const page = getMindfulnessPage(params.slug);
  if (!page) return {};

  return {
    title: page.title,
    description: page.metaDescription,
    alternates: { canonical: `/mindfulness/${params.slug}` },
    openGraph: {
      title: page.title,
      description: page.metaDescription,
      type: 'article',
...
export default function MindfulnessSubPage({ params }) {
  const page = getMindfulnessPage(params.slug);
  if (!page) notFound();
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `https://selfpause.com/mindfulness/${params.slug}`,
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Mindfulness', item: 'https://selfpause.com/mindfulness' },
    ...
    <>
      ...
      <nav aria-label="Breadcrumb" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 lg:pt-28">
        <ol className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <li>/</li>
          <li><Link href="/mindfulness" className="hover:text-teal-500 transition-colors">Mindfulness</Link></li>
          <li>/</li>
      ...
      <section className="py-16 lg:py-20 bg-cream-200">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-8">
            <Link href="/mindfulness" className="text-teal-600 hover:text-teal-700 font-medium">
              ← Browse all mindfulness topics
            </Link>
          </div>
        </div>
      </section>
