...
import { getPositivityPage, getAllPositivitySlugs } from '../../../lib/positivity';
import { notFound } from 'next/navigation';

export async function generateStaticParams() {
  const slugs = getAllPositivitySlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const page = getPositivityPage(params.slug);
  if (!page) return {};

  return {
    title: page.title,
    description: page.metaDescription,
    alternates: { canonical: `/positivity/${params.slug}` },
    openGraph: {
      title: page.title,
      description: page.metaDescription,
      type: 'article',
...
export default function PositivitySubPage({ params }) {
  const page = getPositivityPage(params.slug);
  if (!page) notFound();
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `https://selfpause.com/positivity/${params.slug}`,
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Positivity', item: 'https://selfpause.com/positivity' },
    ...
    <>
      ...
      <nav aria-label="Breadcrumb" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 lg:pt-28">
        <ol className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <li>/</li>
          <li><Link href="/positivity" className="hover:text-teal-500 transition-colors">Positivity</Link></li>
          <li>/</li>
      ...
      <section className="py-16 lg:py-20 bg-cream-200">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-8">
            <Link href="/positivity" className="text-teal-600 hover:text-teal-700 font-medium">
              &larr; Browse all positivity topics
            </Link>
          </div>
        </div>
      </section>
