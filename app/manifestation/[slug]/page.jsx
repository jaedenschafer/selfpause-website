...
import { getManifestationPage, getAllManifestationSlugs } from '../../../lib/manifestation';
import { notFound } from 'next/navigation';

export async function generateStaticParams() {
  const slugs = getAllManifestationSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const page = getManifestationPage(params.slug);
  if (!page) return {};

  return {
    title: page.title,
    description: page.metaDescription,
    alternates: { canonical: `/manifestation/${params.slug}` },
    openGraph: {
      title: page.title,
      description: page.metaDescription,
      type: 'article',
...
export default function ManifestationSubPage({ params }) {
  const page = getManifestationPage(params.slug);
  if (!page) notFound();
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `https://selfpause.com/manifestation/${params.slug}`,
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Manifestation', item: 'https://selfpause.com/manifestation' },
    ...
    <>
      ...
      <nav aria-label="Breadcrumb" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 lg:pt-28">
        <ol className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <li>/</li>
          <li><Link href="/manifestation" className="hover:text-teal-500 transition-colors">Manifestation</Link></li>
          <li>/</li>
      ...
      <section className="py-16 lg:py-20 bg-cream-200">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-navy-600 mb-8">
            Download Selfpause and start building a personalized manifestation and affirmation practice — free.
          </p>
          ...
          <div className="mt-8">
            <Link href="/manifestation" className="text-teal-600 hover:text-teal-700 font-medium">
              &larr; Browse all manifestation topics
            </Link>
          </div>
        </div>
      </section>
