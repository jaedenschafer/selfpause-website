...
import { getAffirmationPage, getAllAffirmationSlugs } from '../../../lib/affirmations';
import { notFound } from 'next/navigation';

export async function generateStaticParams() {
  const slugs = getAllAffirmationSlugs();
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const page = getAffirmationPage(params.slug);
  if (!page) return {};

  return {
    title: page.title,
    description: page.metaDescription,
    alternates: { canonical: `/affirmations/${params.slug}` },
    openGraph: {
      title: page.title,
      description: page.metaDescription,
      type: 'article',
...
export default function AffirmationSubPage({ params }) {
  const page = getAffirmationPage(params.slug);
  if (!page) notFound();
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `https://selfpause.com/affirmations/${params.slug}`,
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Affirmations', item: 'https://selfpause.com/affirmations' },
    ...
    <>
      ...
      <nav aria-label="Breadcrumb" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 lg:pt-28">
        <ol className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <li>/</li>
          <li><Link href="/affirmations" className="hover:text-teal-500 transition-colors">Affirmations</Link></li>
          <li>/</li>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {page.sections.map((section, i) => (
            <div key={i}>
              ...
              {/* Banner ad after the 3rd section */}
              {i === 2 && page.bannerAd && (
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-8">
            <Link href="/affirmations" className="text-teal-600 hover:text-teal-700 font-medium">
              ← Browse all affirmation categories
            </Link>
          </div>
        </div>
      </section>
